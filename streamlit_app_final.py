...
PHOTOS_DIR = "photos"  # Папка с фотографиями в репозитории
...
if st.session_state.stage == 'start':
    ...
    if current_idx < len(products):
        ...
        st.markdown(f"**Группа:** {st.session_state.group_name}")
        st.markdown("---")
    ...
    st.info(f"""
    **Ваш номер:** {st.session_state.participant_number}  
    **Группа:** {st.session_state.group_name}  
    **Оценено товаров:** {len(st.session_state.responses)}
    """)
    
    # Кнопка для скачивания результатов (для администратора)
    if st.checkbox("🔧 Показать результаты (только для администратора)"):
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"results_{st.session_state.participant_number}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv"
        )
